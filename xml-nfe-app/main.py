...
# API KEY GOOGLE MAPS
API_KEY = 'AIzaSyDBiKt0EWsz1aJQbizO7NJJBB-GA8O67x8'
dict_ide = {}
dict_emit = {}
dict_dest = {}
...
def runNFe(Arquivo):
    '''
    Gera um arquivo CSV a partir de um XML de nota fiscal
    '''
    ...
    ########################################################################################################################################################################################################
    # DICIONARIO EMISSOR
    dict_emit = {"CNPJ": None,"xNome": None,"xFant": None,
                 "xLgr": None,"nro": None,"xCpl": None,
                 "xBairro": None,"cMun": None,"xMun": None,
                 "UF": None,"CEP": None,"cPais": None,
                 "xPais": None,"fone": None,"IE": None,
                 "CRT": None, "Arquivo": None}
    ...
    cep_emit = dict_emit["CEP"]
    cnpj = dict_emit.get('CNPJ')
    log = dict_emit.get('xLgr')
    nro = dict_emit.get('nro')
    uf = dict_emit.get('UF')
    end_emit = f'{cnpj} - {log} {nro}, {uf}'
    df_emit = pd.DataFrame(dict_emit, index=[0])

    ########################################################################################################################################################################################################
    # DICIONARIO DESTINATARIO
    dict_dest = {"CNPJ": None,"xNome": None,"xFant": None,"xLgr": None,"nro": None,"xCpl": None,"xBairro": None,"cMun": None,"xMun": None,"UF": None,"CEP": None,"cPais": None,"xPais": None,"fone": None,"IE": None,"CRT": None, "Arquivo": None}

    dict_dest['CNPJ']=nota.infNFe.dest.CNPJ
    dict_dest['xNome']=nota.infNFe.dest.xNome
    dict_dest['xLgr']=nota.infNFe.dest.enderDest.xLgr
    dict_dest['nro']=nota.infNFe.dest.enderDest.nro
    dict_dest['xCpl']=nota.infNFe.dest.enderDest.xCpl
    dict_dest['xBairro']=nota.infNFe.dest.enderDest.xBairro
    ...
    dict_dest['fone']=nota.infNFe.dest.enderDest.fone
    dict_dest['IE']=nota.infNFe.dest.IE
    dict_dest['Arquivo']=Arquivo

    cep_dest = dict_dest["CEP"]
    cnpj = dict_dest.get('CNPJ')
    log = dict_dest.get('xLgr')
    nro = dict_dest.get('nro')
    uf = dict_dest.get('UF')
    end_dest = f'{cnpj} - {log} {nro}, {uf}'
    ...
    df_ = df_emit.merge(df_dest, how="left", on="Arquivo")
    df_ = df_.merge(df_infNFe, how="left", on="Arquivo" )
    # df_ = df_.merge(df_det, how="left", on="Arquivo")

    return df_, cep_emit, cep_dest, end_emit, end_dest


def get_coordinates_from_cep(cep):
    '''
    Retorna as coordenadas de um codigo postal
    '''
    ...
    if geocode_result:
        ...
        raise ValueError(f"Não foi possível encontrar o endereço para o CEP: {cep}")
    

def distance(emit, dest):
    '''
    Calcula a distância entre o emissor e o destinatario da NF
    '''
    return gmaps.distance_matrix(
            origins=emit,
            destinations=dest,
            mode="driving"  # Pode ser alterado para "walking", "bicycling" ou "transit"
        )


if __name__ == '__main__':
    # Inicializa o cliente da API GoogleMAPS
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.xml'):
            ...
            df, cep_emit, cep_dest, end_emit, end_dest = runNFe(file_path)

            emit_location = get_coordinates_from_cep(cep_emit)
            print(f'\tEmissor: CNPJ {end_emit}')
            ...
            # Extrair a distância do resultado
            ...
            #     print(f"A distância entre os dois endereços é de {distance_text} ({distance_value} metros).")
            # else:
